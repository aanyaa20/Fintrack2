      ...
      <div className="max-w-4xl mx-auto px-6 py-12">
        ...
        <div className="space-y-6">
          {posts.map((post, index) => (
            <article key={index} className="bg-[#0f1419] p-6 rounded-lg border border-gray-800 hover:border-green-500 transition-colors cursor-pointer">
              <div className="flex items-center gap-3 mb-3 text-sm text-gray-400">
                <span>{post.date}</span>
                <span>•</span>
                <span>{post.readTime}</span>
              </div>
              <h2 className="text-2xl font-semibold text-white mb-3 hover:text-green-500 transition-colors">
                {post.title}
              </h2>
              <p className="text-gray-300 leading-relaxed mb-4">
                {post.excerpt}
              </p>
              <a href="#" className="text-green-500 hover:text-green-400 font-medium inline-flex items-center gap-1">
                Read more →
              </a>
            </article>
          ))}
        ...
        <div className="mt-12 text-center">
          ...
          <button className="px-6 py-3 bg-green-500 text-gray-900 font-semibold rounded-lg hover:bg-green-400 transition-colors">
